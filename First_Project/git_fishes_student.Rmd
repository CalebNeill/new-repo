...
[My GitHub Repository](https://github.com/CalebNeill/new-repo)
...
It is not a backup of your files, if you lose your computer you will lose any files not backed up on another storage.
```
...
# Write your answer here
Echo determines whether the code is included in the display along with the output. By changing it to FALSE, any code written will not be displayed.
```
...
path <- "/Users/caleb/OneDrive/Documents/GitHub/new-repo/First_Project/data/OA_activitydat_20190302_BIOL3207.csv"
...
path <- "/Users/caleb/OneDrive/Documents/GitHub/new-repo/First_Project/output_data"
...
categories <- data.frame(data$species, data$treatment)

des <- describe(categories)
des
...
## The red highlights indicate what the previous version of the file had written/empty. The green highlight is what was added to the latest version of the file.
```
...
data %>% ggplot(mapping = aes(x = treatment, y = activity, colour = treatment)) +
  geom_boxplot() +
  facet_wrap(~ species) +
...
ggsave(filename = paste("graph1", ".jpeg"), plot = last_plot(), path = "/Users/caleb/OneDrive/Documents/GitHub/new-repo/First_Project/output/figures", device = "png")
```
...
```{r}
## Write your answer here
There appears to be no folder with output/figures and no graph saved into github due to the .gitignore file. There was an error in pushing to origin, which was resolved by ignoring the output/figures folder which I am assuming is was the purpose of gitignore.
```
...
Yes, you can adjust the github to be able to read and track the .png files to not be corrupted when attempting to open them.
```
...
data %>% filter(species %in% c("chromis", "lemon", "acantho")) %>% ggplot(mapping = aes(x = treatment, y = activity, colour = treatment)) + geom_boxplot() + facet_wrap(~ species) + geom_jitter(width = 0.2, height = 0)
```
...
#setting up the vector that will be used for the loop
animal <- unique(data$species)
test_output <- vector('list', length = length(animal))
names(test_output) <- animal

#loop to generate t.tests for each species treatment activity
for (species in animal) {
  species_data <- data[data$species == species,]
  test_output[[species]] <- t.test(formula = activity ~ treatment,
                                   data = species_data,
                                   alternative = 'greater')
}

#Table to create more tidy summary of the t.tests
tab <- map_df(test_output, tidy)
tab$species <- c(animal)
new_tab <- tab %>%
  select(species, everything()) %>% subset(select = -c(estimate, statistic, method, alternative))

colnames(new_tab) <- c("Species", "CO2 mean", "Control mean", "P value", "DF", "Lower Conf", "Upper Conf")
...
#I'm not sure what this means. Does it want the same table I made previously?
...
Mean activity for `r "lemon"` in the control group was `r "new_tab[which(new_tab$Species == "lemon"),c(3)]"` (s / min) compared to the OA treatment group, which was `r "new_tab[which(new_tab$Species == "lemon"),c(2)]"` (s / min). The difference between control and OA treatment means was `r "new_tab[which(new_tab$Species == "lemon"),c(2)] - new_tab[which(new_tab$Species == "lemon"),c(3)]"` (s / min) (95% CI: `r "new_tab[which(new_tab$Species == "lemon"),c(6)]"` to `r "new_tab[which(new_tab$Species == "lemon"),c(7)]"`). 
...
**Write your answer here**
The null hypothesis being tested was that acidification treatment has no effect on fish activity. Based on the results above about the lemon species of fish, the p-value given from the t.test was 'r "new_tab[which(new_tab$Species == "lemon"),c(4)]"', which is < 0.05 and therefore the null hypothesis can be rejected and from the values given, the treatment appears to have a positive effect on the activity of the fish.
